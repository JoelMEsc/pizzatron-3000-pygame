...
class Tablero():
    ...
    def mostrar_orden(self):
        ...
        if len(ingredientes_requeridos) == 1:
            ...
            for i, (tipo, data) in enumerate(configuracion_ingredientes.items()):
                surf = pygame.transform.rotozoom(ingredientes_imagenes[tipo][0], data["angulo"], data["escala"])
                screen.blit(surf, (data["pos_x"], data["pos_y"]))
                screen.blit(font_texto.render(data['texto'], True, "#425756"), (820, 175 + i * 20))
...
def agregar_ingrediente(mouse_pos):
    global ingrediente_sostenido, sonido_salsa_reproduciendo
    if ingrediente_sostenido:
        detener_sonido_salsa()
        ...
        if pizza_debajo:
            if ingrediente_sostenido.tipo.startswith("salsa"):
                ...
                sonidos["ingrediente_agregado"].play() 
            ...
            ingrediente_sostenido.estado = "soltado"
            sonidos["soltar_ingrediente"].play() 
...
def detener_sonido_salsa():
    global sonido_salsa_reproduciendo
    if sonido_salsa_reproduciendo:
        sonido_salsa_reproduciendo.stop()
        sonido_salsa_reproduciendo = None
...
def reproducir_sonido_salsa(tipo):
    global sonido_salsa_reproduciendo
    detener_sonido_salsa()
    sonido_salsa_reproduciendo = sonidos[tipo]
    sonido_salsa_reproduciendo.play(-1)

def revisar_pizza():
    global procesando_resultado, velocidad_cinta, estado_juego
    if pizza.sprite and tablero.orden_actual:

        tiene_salsa = pizza.sprite.salsa and pizza.sprite.salsa.replace("salsa_", "") == tablero.orden_actual["salsa"] and pizza.sprite.nivel_salsa == 4
        if tiene_salsa:
        ...
        if pizza_completa:
            ...
            sonidos["caja_registradora"].play()
...
# Efectos de sonido
sonidos = {
    "caja_registradora": pygame.mixer.Sound("audio/caja_registradora.mp3"),
    "ingrediente_agregado": pygame.mixer.Sound("audio/ingrediente_agregado.mp3"),
    "salsa_normal": pygame.mixer.Sound("audio/salsa_normal.mp3"),
    "salsa_picante": pygame.mixer.Sound("audio/salsa_picante.mp3"),
    "soltar_ingrediente": pygame.mixer.Sound("audio/soltar_ingrediente.mp3")
}
